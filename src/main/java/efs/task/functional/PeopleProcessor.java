...
class PeopleProcessor {
    ...
    // TODO: kontynuuj implementację metody tak, aby dla kolekcji (people) obiektów
    // klasy Person zwracała listę
    ...
    // TODO: kontynuuj implementację metody tak, aby dla kolekcji (people) obiektów
    // klasy Person zwracała listę
    // imion ludzi żyjących w danym mieście (cityOfLiving). Lista ma być posortowana
    // po wieku osób.
    List<String> namesOfPeopleSortedByAgeLivingIn(String cityOfLiving, Collection<Person> people) {
        return people.stream().filter(p -> p.getCityOfLiving().equals(cityOfLiving))
                .sorted(Comparator.comparingInt(Person::getAge)).map(p -> p.getName()).collect(Collectors.toList());
    }

    // TODO: kontynuuj implementację metody tak, aby dla kolekcji (people) obiektów
    // klasy Person zwracała mapę
